...
$pythonVersion = "3.13.2"; $pythonInstaller = "https://www.python.org/ftp/python/$pythonVersion/python-$pythonVersion-amd64.exe"; $packages = "requests", "numpy", "pandas"; 
if (-not (Get-Command python -ErrorAction SilentlyContinue)) {
    Invoke-WebRequest -Uri $pythonInstaller -OutFile "python-installer.exe"
...
if (-Not (Test-Path $sysmonZipPath)) {
    Write-Host "Sysmon ZIP file not downloaded. Please check the URL or your internet connection."
    exit 1
}
